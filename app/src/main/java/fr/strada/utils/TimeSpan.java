...
*/
        if ((millis / TimeSpan.HOURS)<10)
            sb.append("0");
        sb.append(millis / TimeSpan.HOURS);
        millis = millis % TimeSpan.HOURS;
        sb.append(":");
        if ((millis / TimeSpan.MINUTES)<10)
            sb.append("0");
        sb.append(millis / TimeSpan.MINUTES);
...
*/
        ...
        double x = (millis / TimeSpan.MINUTES) ;
        double xx = x / 60 ;
        int y = (int)(xx*10);
        sb.append(y);
    ...
    public static boolean isAfter(TimeSpan first, TimeSpan second) {
        long millis = second.time;
        if (millis < 0) {
            millis = -millis;
        }
        ...
        long hourSecond =  (millis / TimeSpan.HOURS);

        if (first.time >= millis) {
            return true;
        }
        return false;
    }
